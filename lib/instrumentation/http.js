...
const url = require("url"),
  ...
  api = require("../api"),
  httpCommon = require("./http-common"),
...
let instrumentHTTP = (http, opts = {}) => {
  ...
  shimmer.wrap(http, "request", function(original) {
    return function(options, cb) {
      ...
      const initialMeta = Object.assign(
        {
          [schema.EVENT_TYPE]: "http_client",
          [schema.PACKAGE_VERSION]: opts.packageVersion,
          [schema.TRACE_SPAN_NAME]: "http_client",
        },
        httpCommon.httpMetadataFromURLOptions(formatOptions)
      );

      return api.startAsyncSpan(initialMeta, span => {
        let wrapped_cb = function(res) {
          Object.assign(span, httpCommon.httpMetadataFromHttpResponse(res));
